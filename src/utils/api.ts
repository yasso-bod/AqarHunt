import { API_BASE_URL, API_KEY, REQUEST_TIMEOUT } from '../config/api';
...
async function fetchJSON(path: string, init: RequestInit = {}) {
  ...
  const res = await fetchWithTimeout(url, { ...init, headers: getHeaders(init.headers ?? {}) });

  const ct = res.headers.get('content-type') || '';
  if (!ct.includes('application/json')) {
    const body = await res.text();
    throw new APIError('Server returned unexpected response format (expected JSON)', res.status, 'INVALID_RESPONSE_FORMAT', body, url);
  }

  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new APIError(err.detail || `HTTP ${res.status}`, res.status, err.code, JSON.stringify(err), url);
  }
...
export async function apiRequest<T>(endpoint: string, options: RequestInit = {}, skipAuth = false): Promise<T> {
  if (skipAuth) return fetchJSON(endpoint, { ...options, headers: {} });
